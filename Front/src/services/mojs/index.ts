...
export function intro(id: string) {
	...
	const shiftCurve = approximate(
		easing.path('M0,100 C50,100 50,100 50,50 C50,0 50,0 100,0')
	);
	const scaleCurve = approximate(
		easing.path(
	...
	const shapesLogo: Shape[] = [];
	const shapesLines = [];

	const _letters = 'MyLabz';
	...
	const initShapesLogo = (_letter: string, index: number) => {
		shapesLogo.push(
			new Shape({
				...CHAR_OPTS,
				shape: _letter,
	...
	const largeBeaker = new Shape({
		...
		x: 300,
		y: 0,
		scaleY: { 1: 1, curve: approximate(scaledCurve(0.05)) },
		delay: 232,
		duration: 680,
	}).then({
		delay: 115,
	...
	[..._letters].forEach(initShapesLogo);

	timeline.add(largeBeaker, largeBeakerContent);
	timeline.add(shapesLogo);
...
export const animatedLicorne = (): AnimatedIcon => {
	...
	const translationCurve = easing.path(
