...
import { Audio as ThreeAudio, AudioAnalyser, AudioListener, AudioLoader } from "three";
...
export class Sound implements ThreeObject {
    ...
    public load() {
        return new Promise((res, err) => {
            this.audioLoader.load(`/three/KUWAGO-Colors.${this.audioType}`, (buffer) => {
                this.audio.setBuffer(buffer);
                this.audio.setLoop(true);
                this.audio.setVolume(0.5);
                res(true)
            }, () => { console.log('on progress') }, (err) => {
                res(false)
            });
        })
    }
    ...
    public ftt() {
        ...
        const lowerHalfArrayAvg = lowerHalfArray.reduce((a, b) => a + b, 0) / lowerHalfArray.length
        const upperHalfArray =
            dataArray.slice((dataArray.length / 2) - 1, dataArray.length - 1);  // do some basic reductions/normalisations
        const upperHalfArrayAvg = lowerHalfArray.reduce((a, b) => a + b, 0) / lowerHalfArray.length
