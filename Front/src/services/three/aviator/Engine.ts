...
export class Engine implements ThreeObject {
    ...
    public speed = 0;
    public initSpeed = .00035;
    public baseSpeed = .00035;
    public targetBaseSpeed = .00035;
    public incrementSpeedByTime = .0000025;
    public incrementSpeedByLevel = .000005;
    ...
    public rotateEnnemies() {
        for (let i = 0; i < this.ennemiesHolder.ennemiesInUse.length; i++) {
            ...
            const diffPos = this.airPlaine.mesh.position.clone().sub(ennemy.mesh.position.clone());
            const d = diffPos.length();
            if (d < this.ennemyDistanceTolerance) {
                this.particlesHolder.spawnParticles(ennemy.mesh.position.clone(), 15, this.colors.red, 3);

                this.ennemiesPool.unshift(this.ennemiesHolder.ennemiesInUse.splice(i, 1)[0]);
